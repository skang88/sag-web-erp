...
function PackingItemsFetcher() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${API_BASE_URL}/api/packing?${queryParams}`;
        ...
        <table style={tableStyle}>
          <thead>
            <tr style={tableHeaderRowStyle}>
              <th style={tableHeaderStyle}>부품 번호</th>
              <th style={tableHeaderStyle}>아이템 이름</th>
              <th style={tableHeaderStyle}>팔레트 수</th>
              <th style={tableHeaderStyle}>총 중량 (lbs)</th>
              <th style={tableHeaderStyle}>규격 (W x D x H)</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.partNumber} style={tableRowStyle}>
                <td style={tableCellStyle}>{item.partNumber}</td>
                <td style={tableCellStyle}>{item.itemName}</td>
                <td style={tableCellStyle}>{item.palletCount}</td>
                <td style={tableCellStyle}>{item.totalWeight}</td>
