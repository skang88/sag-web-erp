...
exports.sendVerificationEmail = async (email, token) => {
  const verificationUrl = `http://172.16.220.32:8001/api/auth/verify-email?token=${token}`; // BASE_URL 대신 고정 IP 사용 중
...
exports.sendPasswordResetEmail = async (email, resetToken) => {
    ...
    const resetUrl = `http://172.16.220.32:3000/reset-password?token=${resetToken}`; // 예시: 프론트엔드 URL
